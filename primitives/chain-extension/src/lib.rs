...
impl ChainExtension for JupiterExt {
        ...
        <E::T as SysConfig>::AccountId: UncheckedFrom<<E::T as SysConfig>::Hash> + AsRef<[u8]>,
    {
        ...
        if let Some(output) = match func_id {
            ...
            0x4a => jupiter_io::pairing::pairing(0x2a, input).map(b2b),
            0x4b => jupiter_io::pairing::pairing(0x2b, input).map(b2b),
            0x4c => jupiter_io::pairing::pairing(0x2c, input).map(b2b),
            0x4d => jupiter_io::pairing::pairing(0x2d, input).map(b2b),
            // Use megaclite
            ...
            0x7a => curve::pairing(0x2a, input).map(b2b).ok(),
            0x7b => curve::pairing(0x2b, input).map(b2b).ok(),
            0x7c => curve::pairing(0x2c, input).map(b2b).ok(),
            0x7d => curve::pairing(0x2d, input).map(b2b).ok(),
